...
function isTransactionRecent(transaction: Transaction) {
  return Date.now() - transaction.addedTime < (24 * 60 * 60 * 1000) // 1 day
}
...
export function usePendingApproval(tokenAddress?: string, spender?: string): string | undefined {
  const { chainId } = useActiveWeb3React()
  const txs = useAtomValue(transactionsAtom)
  if (!chainId || !tokenAddress || !spender) return undefined
