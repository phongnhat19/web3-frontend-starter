...
export const formatTime = (timeinMs: number, timeFormat = 'HH:mm - dd/MM/yyyy') => {
  return format(
    new Date(timeinMs),
    timeFormat,
  )
...
export const formatUTCTime = (timeInMs: number, timeFormat = 'HH:mm - dd/MM/yyyy') => {
...
export function formatPercent(percentString, fallback = '-') {
  ...
  if (Number(fixedPercent) > 0) {
    if (Number(fixedPercent) > 100) {
      return `${formatNumber(percent?.toFixed(0), false, false).toLocaleString()}%`
    } else {
      return `${formatNumber(fixedPercent, false, false)}%`
    }
  } else {
    return `${formatNumber(fixedPercent, false, false)}%`
  }
...
export const formatNumber = (number: any, usd = false, scale = true, decimals = 0) => {
  ...
  return Numeral(num).format(`0,0.${Array(decimals).fill(0).join('')}`)
}
